function lgraph = vgg16_model_32_blank(numClasses)
    % VGG16 ufficiale MathWorks -> input 32×32 -> testa custom -> reinit totale
    net = vgg16; % usato solo per la topologia
    lgraph = layerGraph(net);
    
    % 1) Cambiare input a 32×32 (nome layer input VGG16: 'input')
    inLayer = imageInputLayer([32 32 3], 'Name','input', 'Normalization','none');
    lgraph  = replaceLayer(lgraph, 'input', inLayer);
    
    % Testa: fc8 -> softmax -> output
    %lgraph = replaceLayer(lgraph, 'fc8', fullyConnectedLayer(numClasses,'Name','fc'));
    %lgraph = replaceLayer(lgraph, 'prob', softmaxLayer('Name','softmax'));
    %lgraph = replaceLayer(lgraph, 'output', classificationLayer('Name','cls'));

    
    % 2) Rimuovere la parte fully-connected originale
    layersToRemove = {'fc6','relu6','drop6', ...
                      'fc7','relu7','drop7', ...
                      'fc8','prob','output'};
    lgraph = removeLayers(lgraph,layersToRemove);
    
    % 3) Crea la nuova "testa"
    newLayers = [
        flattenLayer('Name','flatten')
        fullyConnectedLayer(4096,'Name','fc6_custom')
        reluLayer('Name','relu6_custom')
        dropoutLayer(0.5,'Name','drop6_custom')
        fullyConnectedLayer(4096,'Name','fc7_custom')
        reluLayer('Name','relu7_custom')
        dropoutLayer(0.5,'Name','drop7_custom')
        fullyConnectedLayer(numClasses,'Name','fc8_custom')
        softmaxLayer('Name','softmax')
        classificationLayer('Name','cls')
        ];
    
    % 4) Connettere la nuova testa all'ultimo layer convolutivo
