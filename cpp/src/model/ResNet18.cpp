    ...
    std::vector<int64_t> replace_stride_with_dilation)
{
    ...
    m_fc = register_module(
        "fc", torch::nn::Linear(512 * BasicBlock::m_expansion, num_classes));
    ...
    bool dilate)
{
    ...
    if ((stride != 1) || (m_inplanes != planes * BasicBlock::m_expansion))
    {
        downsample = torch::nn::Sequential(
            torch::nn::Conv2d(create_conv1x1_options(
                m_inplanes, planes * BasicBlock::m_expansion, stride)),
            torch::nn::BatchNorm2d(planes * BasicBlock::m_expansion));
    }
...
build_resnet18(int64_t num_classes, bool zero_init_residual,
         int64_t groups, int64_t width_per_group,
         std::vector<int64_t> replace_stride_with_dilation)
{
    const std::vector<int64_t> layers{2, 2, 2, 2};
    std::shared_ptr<ResNet18> model = std::make_shared<ResNet18>(
