import torch
from torchvision import models
import sys

def export_vgg16(output_name = "vgg16", num_classes: int = 100, pretrained_weights = None):
    for model, name in (
            (models.vgg16(weights=pretrained_weights), output_name),
    ):
        model.classifier[6] = torch.nn.Linear(4096, num_classes) # TODO: in_feature?

        if not pretrained_weights:
            def init_weights(m):
                if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Linear):
                    torch.nn.init.kaiming_normal_(m.weight)
                    if m.bias is not None:
                        torch.nn.init.zeros_(m.bias)

            model.apply(init_weights)

        traced_model = torch.jit.script(model)
        traced_model.save(f"{name}.pt")


if __name__ == "__main__":
    if len(sys.argv) == 1:
        params = {}
    elif len(sys.argv) == 2:
        params = {"output_name": sys.argv[1]}
    else: # len(sys.argv) == 3 or more
        params = {"output_name": sys.argv[1], "num_classes": int(sys.argv[2])}

    export_vgg16(**(params)) # weights=models.ResNet18_Weights.IMAGENET1K_V1 #