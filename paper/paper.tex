...
\textbf{Results.}
The design yields \vRuns{} runs and \vBlocks{} doubly instrumented measurement blocks. Between-run repeatability is high: the median coefficient of variation of training energy is \vCVTrainMedian\,\%. Under a common measurement boundary, training energy differs across ecosystems by \vSpreadTrainMin$\times$ to \vSpreadTrainMax$\times$ depending on the block, while final accuracy converges to within \vAccSpreadFashion{} percentage points on Fashion-MNIST --- the differences are energetic, not qualitative. Measurement coverage, however, is itself unequal: tracked time is \vCoverageMin\,\% to \vCoverageMax\,\% of a run's wall time depending on the stack, and charging the untracked remainder at its upper bound leaves the ecosystem order unchanged in \vRankStableBlocks{} of \vRankStableTotal{} blocks while compressing the spread to \vSpreadJobMin$\times$--\vSpreadJobMax$\times$. We therefore report which ecosystem is cheaper with confidence, and by how much only as a range. The two instruments agree on energy to \vCCmeasDisagreementPct\,\% over the whole campaign --- \vAgreeLongPct\,\% for blocks longer than \SI{30}{\second}, degrading to \vAgreeShortPct\,\% below one second, where the block is shorter than the sampling interval --- so the difficulty with software estimation is narrower and stranger than inaccuracy: the duration CodeCarbon reports beside each energy figure is not the duration the energy was accumulated over, but $\max(\text{phase},\ \vWindowFloorS\,\text{s})$, a relation that explains $R^2 = \vWindowModelRsq$ of its variance. Any power or energy--time analysis built on that field therefore understates power by up to \vPowerUnderstatedSubSecond$\times$ for sub-second phases while being exact above ten seconds --- a bias that falls precisely on the fastest ecosystems and the inference phase.
...
Our results are correspondingly more modest and more defensible than a bare ranking. Between-run repeatability is high --- the median coefficient of variation of training energy across \vRuns{} runs is \vCVTrainMedian\,\% --- so the intervals we report are genuine. Under a common measurement boundary, training energy differs across ecosystems by \vSpreadTrainMin$\times$ to \vSpreadTrainMax$\times$ depending on architecture and dataset, with \vSpreadTrainBest{} consistently cheapest, while final test accuracy converges to within \vAccSpreadFashion{} percentage points on Fashion-MNIST across all \vEcosystems{} stacks. The ecosystem effect is real, it is large, and it is energetic rather than qualitative --- though not as large as a per-phase comparison alone suggests, for a reason we return to in Section~\ref{sec:instrument}: the stacks differ in how much of their wall time falls inside a measured block at all, and charging the remainder compresses the spread to \vSpreadJobMin$\times$--\vSpreadJobMax$\times$ without reordering it.
...
well resolved. The factors are per-phase and are an upper bound: charging the
energy that falls between measured phases compresses them to
\vSpreadJobMin$\times$--\vSpreadJobMax$\times$ without changing the order
(Section~\ref{sec:instrument}). Final accuracy converges to within \vAccSpreadFashion{}
...
than the average. CodeCarbon samples at a fixed interval ---
\vSamplingIntervalS\,s in our configuration --- and when a block is long
relative to that interval the two instruments are reading the same accumulating
counters: above \SI{30}{\second} they agree to \vAgreeLongPct\,\%. When a block
is shorter than the sampling interval there is nothing to accumulate, the
estimator interpolates from sampled power, and the mean error rises to
\vAgreeShortPct\,\% with a worst case of \vAgreeShortWorstPct\,\%. The visible
structure in Figure~\ref{fig:instrument}a is that transition.
...
\textbf{What the boundary leaves out, and how much it matters.}
Energy is attributed only to the phases each stack brackets. Whatever a stack
does \emph{between} them --- rebuilding a loader, compiling a graph,
synchronising, reshuffling on the host --- consumes real energy and appears in no
measurement. That would be unimportant if the fraction were similar across
stacks. It is not. Measured over each run's wall time, tracked time ranges from
\vCoverageMin\,\% (\vCoverageWorstEco) to \vCoverageMax\,\%
(\vCoverageBestEco). More than half of a \vCoverageWorstEco{} run happens
outside any measured block.

This flatters the stacks that leave the most time untracked, and by a large
factor, so the comparison cannot simply be reported and qualified. We bound it
instead. Job-level energy is the tracked energy plus the untracked time
multiplied by whatever power the gaps draw, and that power is bracketed on both
sides without any unmeasured assumption: it cannot be negative, and it cannot
exceed the lowest power observed in any \emph{measured} block of the same stack,
because a gap does strictly less work than the least busy block. Charging the
gaps at that upper bound inflates a run's energy by at most
\vInflationMax$\times$ --- for \vCoverageWorstEco, and by nothing at all for
\vCoverageBestEco.

The ranking survives; the effect size does not.
Table~\ref{tab:coverage} reports the ecosystem order at each end of that range.
It is \emph{identical} in \vRankStableBlocks{} of \vRankStableTotal{} blocks and
differs by a single adjacent transposition in the other two, with rank
correlation no lower than \vRankStableRhoMin. Since the order is the same at both
ends of the admissible range, it is the same everywhere in between, and does not
depend on a quantity we did not measure. The \emph{magnitude}, however, is
boundary-dependent: the training spread of
\vSpreadTrainMin$\times$--\vSpreadTrainMax$\times$ per phase becomes
\vSpreadJobMin$\times$--\vSpreadJobMax$\times$ once the gaps are charged at their
upper bound. The true figure lies between the two and nearer the per-phase end,
since gaps draw less than the least busy block. We therefore report which
ecosystem is cheaper with confidence and \emph{by how much} only as a range.

\begin{table}[t]
\centering
\caption{Ecosystem order under the two bounds on gap power, per block, and the
spread each implies. The order is what survives; the factor is not.}
\label{tab:coverage}
...
\vSpreadTrainMax$\times$ more energy on one ecosystem than another per measured
phase, or \vSpreadJobMin$\times$ to \vSpreadJobMax$\times$ once the time between
phases is charged at its upper bound. The ordering is the same either way. Three of the
...
\begin{itemize}
 ...
 mandate when the cheaper stack has a tenth of the ecosystem around it, and the
 gap a practitioner will actually see is nearer the job-level figure than the
 per-phase one.
...
yes \\
\addlinespace
Reported duration is a floor &
\(\max(\text{phase},\ \vWindowFloorS\,\text{s})\); power understated \vPowerUnderstatedSubSecond$\times$ below one second, exact above ten &
no \\
\addlinespace
...
of the two: it is $\max(\text{phase},\ \vWindowFloorS\,\text{s})$ rather than the
accumulation window, so any power or energy--time quantity derived from it is
biased as a function of phase length. We use counter-bracketed durations
...
Five repetitions is a small sample for a rank test, and we have preferred
effect sizes (Cliff's delta) and Holm-corrected pairwise comparisons over
unadjusted p-values accordingly. Between-run variability is low --- a median
coefficient of variation of \vCVTrainMedian\,\% for training energy --- which
makes the design well powered for the effect sizes we report, and we note that
low variance is a statement about the measurement rather than a guarantee that
the measurement is of the right thing.
...
ecosystem than another per measured phase, and between \vSpreadJobMin$\times$
and \vSpreadJobMax$\times$ once the time between phases is charged, with
between-run variability of order \vCVTrainMedian\,\%. The order is the same under
both boundaries; the factor is not, which is why we give it as a range. Because three of the ecosystems compared execute a
...
But the duration it reports beside that energy is a floor of about
\vWindowFloorS\,s rather than the interval the energy was accumulated over, so
power derived from the two fields is understated by up to
