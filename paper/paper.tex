...
\SI{231}{\watt} against \SI{107}{\watt} of modelled host power; up to \SI{70}{\percent} of reported energy &
no \\
\addlinespace
Mixed tracking mode &
...
The instrument's version becomes a property of the shell &
no \\
\addlinespace
...
0 to 96 threads; \(\rho=-0.73\) against duration, \(11.6\times\) spread &
no \\
\addlinespace
...
campaign it correlates with epoch duration at $\rho = -0.73$, spanning
\(11.6\times\) --- which is to say that a column no one would think to report
tracks the published ranking better than the programming language does. One
...
pipeline; and 56 automated checks verify all of it against the source. Two
