...
The specification is enforced by 57 automated checks that read the source of
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item the \textbf{conformance checker}, 56 executable checks that read the
...
collapse count alone would have absorbed. \vSigPipelineN{} further runs met the
chance-accuracy criterion with training loss falling by more than
\vSigPipelineLossDropMin\,\%: one Rust binary carried a private copy of the input
transform, applied on the training path only, which resized the tensor its
loader had already produced using a function that returns \code{uint8}. Every
value in $[0,1]$ truncated to zero, so it trained on black images and was
evaluated on real ones. Filed as a collapse it would have read as ``VGG-16
sometimes fails to train''; filed correctly it is a defect, and it is fixed. The
\vCollapsedVgg{} collapses reported above are the ones that survive this test.
...
\textbf{Some of them are ours.} Five entries were introduced while repairing the
study, and each was caught by a mechanism rather than by attention: the
conformance checks, the accuracy metric, or the second instrument.

One of the five deserves singling out, because it is not a mistake in the usual
...
pipeline; and 57 automated checks verify all of it against the source. Two
