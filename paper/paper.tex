...
\textbf{What the boundary leaves out.} Energy is attributed only to the phases
each stack brackets, so work a stack does \emph{between} phases --- rebuilding a
loader, synchronising, reshuffling on the host --- consumes real energy and is
charged to nobody. If that fraction differed sharply between ecosystems it would
flatter whichever stack does more of its work outside its own phase boundaries.
It does differ, and we report it: measured from the first block of a run to the
last, tracked time is \vCoverageMin\,\% to \vCoverageMax\,\% of the span, with
\vCoverageWorstEco{} lowest. The comparison is therefore of per-epoch phase
energy and not of end-to-end job energy, and the two differ by up to a fifth for
some stacks. We prefer the phase boundary because it is the quantity a
practitioner can act on, but a study reporting job-level energy would rank these
ecosystems somewhat differently, and should say which it measured.
...
\begin{itemize}
 ...
 hiring, library availability and time to delivery; a \vSpreadTrainMax$\times$
 energy gap is not a mandate when the cheaper stack has a tenth of the
 ecosystem around it.
