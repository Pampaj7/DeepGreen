...
# Pattern nuovo: resnet18_cifar100_eval_epoch1.csv
FILENAME_PATTERN = re.compile(
    r"(?P<model>\w+?)_(?P<dataset>[\w\d]+)_(?P<phase>train|eval)_epoch(?P<epoch>\d+)\.csv"
)
...
if not groups:
    ...
    for (model, dataset), file_phase_list in groups.items():
        ...
        records = []
        for fpath, phase, epoch in sorted(file_phase_list, key=lambda x: x[2]):
