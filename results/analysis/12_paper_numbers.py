...
def industrial_scenario() -> None:
    ...
    The scenario is a scaling of measured *relative* differences, so it is only
    as good as those; regenerating it here keeps it from drifting away from the
    data the way a hand-typed table does.
    """
    runs = pd.read_csv(TABLES / "v2_run_totals.csv")
    infer = runs[runs.phase == "Inference"]
    mean_j = infer.groupby("ecosystem").energy_j.mean()
