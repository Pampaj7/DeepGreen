...
def instrument_facts(epochs: pd.DataFrame) -> None:
    ...
    cov = pd.read_csv(TABLES / "v2_coverage_sensitivity_by_ecosystem.csv")
    ...
    macro("vInflationMax", num(cov.max_inflation.max(), 2))

    rs = pd.read_csv(TABLES / "v2_coverage_sensitivity_ranking.csv")
    lines = [
        r"% generated by results/analysis/12_paper_numbers.py -- do not edit",
        r"\begin{tabular}{llrrc}",
        r"\toprule",
        r"\textbf{Model} & \textbf{Dataset} & \textbf{Spread, per phase} "
        r"& \textbf{Spread, gaps charged} & \textbf{Same order?} \\",
        r"\midrule",
    ]
    for _, row in rs.iterrows():
        same = "yes" if row.identical else f"$\\rho={row.spearman_rho:.2f}$"
        lines.append(
            f"{MODEL[row.model]} & {DATASET[row.dataset]} & "
            f"{row.spread_per_phase:.1f}$\\times$ & "
            f"{row.spread_job_upper:.1f}$\\times$ & {same} \\\\")
    lines += [r"\bottomrule", r"\end{tabular}"]
    (OUT / "tab_coverage.tex").write_text("\n".join(lines) + "\n")
    print("  wrote paper/generated/tab_coverage.tex")

    macro("vRankStableBlocks", int(rs.identical.sum()))
    macro("vRankStableTotal", len(rs))
    macro("vRankStableRhoMin", num(rs.spearman_rho.min(), 2))
    macro("vSpreadJobMin", num(rs.spread_job_upper.min(), 1))
    macro("vSpreadJobMax", num(rs.spread_job_upper.max(), 1))
